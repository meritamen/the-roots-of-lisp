...
initialEnvCtx  = newIORef M.empty >>= flip bindVars (M.toList (LispPrimitive <$> primitives))
...
eval (LispList (LispSymbol "cond":clauses)) = do
  case listToMaybe clauses of
    ...
    Just badForm -> throwError $ LispErrorBadSpecialForm "Unrecognized special form" badForm
...
getVar varname = do
  ...
  case M.lookup varname envCtx of
    ...
    Nothing -> throwError $ LispErrorUnboundVar "Getting an unbound variable" varname
...
setVar varname value = do
  ...
  case M.lookup varname envCtx of
    ...
    Nothing -> throwError $ LispErrorUnboundVar "Setting an unbound variable" varname
  return value
...
bindVars envCtxRef bindings
  ...
  where
    extendEnvCtx bindings envCtx = liftM (M.union envCtx) $ M.fromList <$> mapM addBinding bindings
...
eq badArgList = throwError $ LispErrorWrongArgNumber 2 badArgList
